...
<div style="padding-left: 10%; margin-bottom: 0.2%; border: solid 0px red;">
	...
	<%-- 
		==== #134. 파일첨부하기 ==== 
		먼저, 위의  <form name="writeFrm"> 을 주석 처리한 후 아래와 같이 한다. 
		enctype="multipart/form-data"를 해주어야만 파일첨부가 된다. 
	--%>
	<form name="writeFrm" enctype="multipart/form-data">
		...
		--%>	
         	...
         	<tr>
         		<th>파일첨부</th>
         		<td><input type="file" name="attach" /></td>	<!-- name : VO 속에 추가되어진 attach  -->
         	</tr>
		...
		<!-- 데이터베이스에 이제 insert를 해야 한다. 원글쓰기인지 답변글쓰기인지 구분을 지어줘야 한다. -->		
		<!-- ==== #122. 답변글쓰기 인 경우 
				    부모글의 seq값은 fk_seq값과
				    부모글의 groupno 값과
				    부모글의 depthno 값을 hidden 타입으로 보내준다. 그래야만 데이터베이스에 insert가 된다.  -->
		<input type="hidden" name="fk_seq" value="${fk_seq}" />		<!-- value는 이미 컨트롤러에서 키값으로 넘긴게 있다. -->
